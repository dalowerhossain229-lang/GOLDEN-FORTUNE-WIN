...
const slotSymbolsPool = ["CAISHEN", "COIN", "DRAGON", "LANTERN", "FIREWORK"];

// 💰 ১. লাইভ অ্যাকাউন্ট ব্যালেন্স নিয়ে আসার ডেডিকেটেড গেটওয়ে
app.get('/api/fortunegod-balance', async (req, res) => {
...
app.post('/api/fortunegod-spin', async (req, res) => {
    ...
    try {
        ...
        let adminTriggeredPrize = (balCheck.data && balCheck.data.fortunegod_target) ? balCheck.data.fortunegod_target : null;
        ...
        while (isLoopActive && loopSafety < 200) {
            ...
            if (r1 === r2 && r2 === r3) {
                finalStatus = "win";
                // ৩টি একই চিহ্ন মিললে রাজকীয় ধন-দেবতা মেগা জ্যাকপট গুণিতক চাবি ভাই ভাই
                if (r1 === "CAISHEN") winMultiplier = 30.00;
                else if (r1 === "COIN") winMultiplier = 20.00;
                else if (r1 === "DRAGON") winMultiplier = 12.00;
                else if (r1 === "LANTERN") winMultiplier = 6.00;
            ...
            if (adminTriggeredPrize) {
                ...
                if (adminTriggeredPrize === "force_win" && finalStatus === "win" && winMultiplier >= 4) isLoopActive = false;
            } else {
        ...
        console.error("Fortune God Slot Core Engine Error:", e.message);
...
io.on('connection', (socket) => { console.log("Player connected to Golden Fortune God Slot Engine!"); });

// ২৩ নম্বর গেম ৩০০০০ এ চলছে, তাই ২৪ নম্বর ধন-দেবতার এশিয়ান স্লট গেম প্রজেক্টের স্বাধীন কাস্টম পোর্ট ৩১০০০ কড়া লক হলো ভাই ভাই!
const PORT = process.env.PORT || 31000;
server.listen(PORT, () => { console.log(`🎡 Golden Fortune God Slot Engine Running on port ${PORT}`); });
